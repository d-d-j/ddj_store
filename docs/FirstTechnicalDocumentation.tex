...
\section{Master}
	...
	\subsubsection{Insert}
		\begin{enumerate}
			\item Po przez RESTA dostajemy request od użytkownika. Wewnątrz metody obsługującej odbywa się parsowanie JSONa na wewnętrzne DTO i opakowanie go w Taska.
			\item Task zostaje wysłany do WorkerBalancera gdzie zostaje przesłany do odpowiedniego Workera.
			\item Worker bierze tego Taska (jest to insert) pobiera z NodeBalancera do którego Noda ma go wysłać. 
			\item Następnie serializuje Taska i wysyła. 
			\item Nie czeka na odpowiedź tylko od razu wysyła że jest ok. 
			\item I tu kończy się przetwarzanie inserta 
		\end{enumerate}
	\subsubsection{Select}
		\begin{enumerate}
			...
			\item Rejestruje w TaskManagerze że jest task o takim ID który czeka na tyle odpowiedzi i jego kanał zwrotny na którym słucha. 
			\item Worker bierze tego Taska (był to select) pobiera z NodeBalancera listę nodów i wysyła do nich to żądanie. 
			\item Node odebrał już taska i zrobił z nim co trzeba. 
			\item Node zwraca wynik opakowany w header z ID jest to odebrane przez NodeReciviera 
			\item NodeRecivier deserializuje wiadomość 
			\item NodeRecivier odczytuje z TaskManagera do którego Workera wysłać to co odebrał 
			\item W trakcie tego sprawdzania zminiejszamy liczbę odpowiedzi na które czeka Master dla danego taska
			i jeśli jest = 0 to kasujemy go z managera albo oznaczamy jako zakonczony czy jak tam chcemy. 
			\item Worker odebrał już wyniki od wszystkich nodów 
			\item Worker robi redukcję 12 Worker wysyła wynik na kanał który dostał razem z Taskiem 
			\item Na tym kanale słucha API i zwraca wynik działania
		\end{enumerate}
	\subsubsection{Komponenty}
		\begin{description}
		    \item[NodeBalancer] tutaj chyba powinna być lista nodów albo kolejka priorytetowa. Nie wiem jak ona powinna być odświrzana ale jakoś musi być.
		    NodeBalancer musi pobierać jakieś informacje o stanie nodów i na tej podstawie zmieniać noda na którego wrzucimy dane
		    \item[WorkerPool] ja bym tu dał liste cykliczną bo nasze zapytania powinny działać szybciej niż to zmiana priorytetu. 
		    Nie wiem jeszcze jak oni w tym przykładzie robili to że na jednym kanale wisiało 10 tasków. Można to osiągnąć przez buforowany kanał 
		    i tu trzeba by przetestować jaka konfiguracja jest najlepsza w sensie workerzy / rozmiar bufora (potrzebne dobre testy!)
		    \item[TaskManager] to musi być jakiś słownik ale thread safe. Nie mam pomysłu jak to dobrze ogarnąć żeby nasze taski nie czekały w tym miejscu
		    Dobrze by było gdyby TaskManager trzymał info o wszystkich taskach żeby w każdej chwili można było poejrzeć co sie dzieje
		    kiedy dane zlecenie zostało zakończone itp.
		\end{description}					
