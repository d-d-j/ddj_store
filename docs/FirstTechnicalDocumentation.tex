...
\section{Master}
	...
	\subsection{Przepływ danych}
		...
		\begin{enumerate}
			\item Serwer wystawiający RESTowe API otrzymuje zapytanie select lub insert które kieruje jako request to Task Manager'a
			\item Task Manager tworzy zadanie dla danego zapytania i przekazuje go do wykonania Task Executorowi
			\item Ten odpala wykonanie zapytania w wątku (GO routine)
			\item Go routine przetwarzający dane zapytanie zleca wysłanie utworzonego obiektu TaskRequest do odpowiedniego węzła 
				(wyznaczeniem odpowiedniego węzła zajmować się będzie LoadBalancer)
		...
		\begin{enumerate}
			\item Node communication server odbiera obiekt TaskResponse który kieruje go Task Executora
			...
			\item Jeśli wszystkie węzły na które zostało wysłane zapytanie odpowiedziały, zapytanie jest agregowane (jeśli to 
				potrzebne) a zadanie jest oznaczane jako ukończone
