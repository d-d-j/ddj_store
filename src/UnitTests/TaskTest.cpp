...
TEST_F(TaskTest, SetResult_MultipleResults_Info_Success)
{
	...
	store::storeNodeInfo* info_1 = new store::storeNodeInfo(1, 199, 198, 177, 1,
			1);
	store::storeNodeInfo* info_2 = new store::storeNodeInfo(2, 299, 28, 277,
			266, 2);
	...
	taskResult expected_result(1, TaskType::Info, nullptr, 3 * infoSize,
			(char*) "success_1;success_2;success_3;");
	...
	EXPECT_EQ(*info_2,
			((store::storeNodeInfo* )task.GetResult()->result_data)[1]);
	EXPECT_EQ(*info_3,
			((store::storeNodeInfo* )task.GetResult()->result_data)[2]);
}
